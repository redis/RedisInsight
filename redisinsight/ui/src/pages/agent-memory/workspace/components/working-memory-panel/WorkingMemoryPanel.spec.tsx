...
describe('WorkingMemoryPanel', () => {
  ...
  it('should render the running summary with its header meta when present', () => {
    const summaryText = faker.lorem.paragraph()
    renderComponent(undefined, {
      data: {
        sessionId: mockedSessionId,
        messages: [],
        summary: { text: summaryText, summarizedEvents: 4 },
      },
    })
    ...
    expect(screen.getByTestId('working-memory-summary-meta')).toHaveTextContent(
      '4 messages summarized',
    )
  })
  ...
  describe('add event', () => {
    it('should always render the add event button', () => {
      renderComponent()
