    ...
    <S.CardMeta>
      ...
      {message.createdAt && (
        <TimestampWithRelative
          ...
          content={`Created: ${relativeTime(message.createdAt)}`}
        />
      )}
    </S.CardMeta>
...
/** The running summary, shown as the first card in the working-memory list. */
const SummaryCard = ({ summary }: { summary: AgentMemorySummary }) => (
  <S.Card data-testid="working-memory-summary">
    <S.CardMeta>
      <S.RoleTag $role="summary">Summary</S.RoleTag>
      {summary.updatedAt && (
        <TimestampWithRelative
          dateTime={summary.updatedAt}
          content={`updated ${relativeTime(summary.updatedAt)}`}
        />
      )}
      {summary.summarizedEvents !== undefined && (
        <S.SummaryMeta data-testid="working-memory-summary-meta">
          {pluralize(summary.summarizedEvents, 'message')} summarized
        </S.SummaryMeta>
      )}
    </S.CardMeta>
    <S.CardText>{summary.text}</S.CardText>
  </S.Card>
)
        ...
        <S.PaneMetaRow align="center" gap="l" data-testid="working-memory-meta">
          {!!sessionId && !!data ? (
            <>
              ...
              {data.namespace && (
                <S.MetaItem data-testid="working-memory-namespace">
                  namespace:<code>{data.namespace}</code>
                </S.MetaItem>
              )}
              <S.MetaRowActions>
                <PopoverDelete
