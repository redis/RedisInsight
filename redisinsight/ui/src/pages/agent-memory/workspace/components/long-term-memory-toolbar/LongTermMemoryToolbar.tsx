...
const LongTermMemoryToolbar = () => {
  ...
  const { users, namespaces: knownNamespaces } = useAppSelector(
    agentMemoryFiltersSelector,
  )

  // The server has no enumeration endpoint for topics or the sessions a
  // record was extracted from - offer the values visible in the current
  // result set (records span any owner) plus anything already picked. The
  // shared working-memory session list can't be used: it is scoped to one
  // owner and holds only live sessions, while records outlive them.
    ...
    <S.Toolbar data-testid="long-term-memory-toolbar">
      ...
      <S.FilterRow align="center" wrap>
        ...
        <FilterDropdown
          ...
          selected={sessionIds}
          emptyText="no sessions in results"
