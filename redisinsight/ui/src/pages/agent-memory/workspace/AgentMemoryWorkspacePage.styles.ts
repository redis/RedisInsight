 ...
 * the accent colors (role tags, chips, type badges) are custom, with
 * dark-mode variants applied when the app theme is dark.
 */
const staticAccents = {
  accent: '#dcff1c',
  dark: '#2d4754',
...
/* Odd px values below have no theme.core.space equivalent. */
...
/* The inspector's icon, sitting left of the
 * "Redis Agent Memory / <host>" breadcrumb */
export const HeaderIcon = styled.img<ImgHTMLAttributes<HTMLImageElement>>`
  width: 28px;
  height: 28px;
  flex-shrink: 0;
  /* invert the duotone icon on dark surfaces */
...
>`
  ...
  background: ${({ $role }) => {
    if ($role === 'summary') return palette.summaryHash
    if ($role === 'user' || $role === 'assistant') return palette.roleAccentBg
    return palette.bgTertiary
  }};
  color: ${({ $role }) => {
    if ($role === 'summary') return '#fff'
    if ($role === 'user' || $role === 'assistant') return palette.roleAccent
    return palette.textSecondary
  }};
`

/* Meta line in a working-memory summary card header - folded-in event
 * count and last-updated time, right-aligned next to the Summary badge */
export const SummaryMeta = styled.span<HTMLAttributes<HTMLSpanElement>>`
  margin-left: auto;
  font-family: ${fontMono};
  font-size: ${({ theme }) => theme.core.font.fontSize.s10};
  color: ${palette.textMuted};
