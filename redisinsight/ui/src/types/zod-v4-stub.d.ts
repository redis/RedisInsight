/* eslint-disable @typescript-eslint/naming-convention -- these names must
   match zod's own real exports (enum_/null_/void_/_default, chosen by zod
   to dodge the reserved words enum/null/void/default) verbatim, since
   @rdi-ui/sdk's declaration files reference them by exactly these names */
/**
 * Remove this stub once RedisInsight upgrades TypeScript to 5.x everywhere
 * (root, redisinsight/api, redisinsight/desktop) and re-check whether it's
 * still needed. Re-evaluate then whether @rdi-ui/sdk's zod version/usage
 * still needs this workaround at all.
 *
 * Why this exists:
