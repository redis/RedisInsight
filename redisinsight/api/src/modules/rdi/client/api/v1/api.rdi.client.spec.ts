...
describe('ApiRdiClient', () => {
  ...
  describe('proxyRequest', () => {
    ...
    it('should return the raw 3xx status without following it server-side, rewriting Location through the proxy', async () => {
      mockedAxios.request.mockResolvedValueOnce({
        status: 307,
      ...
      expect(result.headers.location).toBe(
        '/api/rdi/rdiId/proxy/api/v1/pipelines/new',
      );
    });
    ...
    it('should rewrite a relative redirect Location that stays within the rdi url subpath through the proxy', async () => {
      const scopedClient = new ApiRdiClient(mockRdiClientMetadata, {
        ...mockRdi,
        url: 'http://localhost:4000/rdi',
      });
