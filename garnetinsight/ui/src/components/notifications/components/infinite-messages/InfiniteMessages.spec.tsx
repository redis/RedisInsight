...
describe('INFINITE_MESSAGES', () => {
  ...
  describe('PENDING_CREATE_DB', () => {
    it('should render message', async () => {
      ...
      const description = await screen.findByText(
        /This may take several minutes, but it is totally worth it!\s*You can continue working in Garnet Insight, and we will notify you once done\./,
      )
      const closeButton = await screen.findByRole('button', { name: /close/i })
  ...
  describe('DATABASE_EXISTS', () => {
    it('should render message', async () => {
      ...
      const description = await screen.findByText(
        'Do you want to import your existing database into Garnet Insight?',
      )
  ...
  describe('APP_UPDATE_AVAILABLE', () => {
    it('should render message', async () => {
      ...
      const description = await screen.findByText(
        /With Garnet Insight <version> you have access to new useful features and optimizations\.\s*Restart Garnet Insight to install updates\./,
      )
